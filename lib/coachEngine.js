...
async function webSearch(query) {
  ...
  if (!key) {
    return {
      live: false,
      note: 'Live web search is not configured for this deployment (no SERPER_API_KEY). Answer from general knowledge and say so explicitly.',
    };
  }
  ...
  if (!resp.ok) return { live: false, note: 'Web search request failed.' };
  const data = await resp.json();
  const snippets = (data.organic || []).slice(0, 5).map((r) => `${r.title}: ${r.snippet}`);
  return { live: true, results: snippets };
}
...
const TOOLS = [
  {
    name: 'web_search',
    description:
      "Search the live web. Use this for anything time-sensitive or restaurant/menu-specific, and for finding reputable recipe or workout-plan resources to share (MedFit doesn't maintain its own recipe/workout library).",
    input_schema: {
      type: 'object',
...
function scriptedReply(message, memberWeekNumber) {
...
async function getCoachReply({ message, history, memberWeekNumber }) {
  if (process.env.ANTHROPIC_API_KEY) {
    try {
      ...
      return { reply: scriptedReply(message, memberWeekNumber), live: false, fallbackReason: err.message };
    }
  }
  return { reply: scriptedReply(message, memberWeekNumber), live: false };
}
